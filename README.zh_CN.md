...
1080p @ 60fps。WASM 体积 262KB,gzip 后约 97KB。零依赖。无需特殊服务器标头。
...
- `wasm/hevc-decode.wasm` — WASM 二进制文件(262KB,gzip 后约 97KB)
...
- **体积** — 编译为 WASM 后只有 262 KB(gzip 后约 97 KB),而 libde265 编译到 WASM 大约 2 MB。**小 8 倍** — 这在向浏览器、microVM 或沙盒运行时分发时很关键。
...
# 输出:build-wasm/hevc-decode.js + hevc-decode.wasm(262KB)
```
...
WASM 解码器的性能在原生 C++ 性能的 20% 以内,在两者都编译为 WASM 时,达到了 **libde265 速度的 83%**(libde265 是一个成熟的、经过 10 年优化的 HEVC 解码器)— 而二进制大小只有 **1/8**(262 KB vs ~2 MB;gzip 后约 97 KB)。
