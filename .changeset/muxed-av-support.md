...
- Validated end-to-end: the muxed test stream plays with both video and audio (and seeks) through hls.js.

Scope: the muxed path runs on the main thread (the worker fast path stays video-only), and only AAC audio pass-through is implemented. The Shaka transmuxer still reports muxed A/V mimes as unsupported (separate integration path — follow-up).
