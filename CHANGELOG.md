...
- **`pic_output_flag` ignored** (`decoder.cpp`): the flag parsed from the slice header was overwritten with `true` after each picture, so pictures the bitstream marks as not for output were emitted, and their storage buffer was held for a bump that never came.
